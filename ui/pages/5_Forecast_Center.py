...
def get_station_locations() -> pd.DataFrame:
    ...
    gdf["name"] = gdf["ID"].astype(str).map(station_names).fillna(
        gdf["ID"].astype(str)
    )
...
def _build_header() -> None:
    ...
    st.markdown(
        """
        <div class="main-header">Forecast Center</div>
        <div class="sub-header">
            Operational flood risk monitoring driven by ICON ensemble forecasts
            and the calibrated two-stage LSTM hydrological model.
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def _map_section(
    config: Dict[str, Any], predictions: pd.DataFrame, alert_threshold: float
) -> None:
    ui_cfg = config.get("ui", {})
...
def _hydrograph_section(
    ...
    if "time" in rain_station.columns:
        rain_station = rain_station.groupby("time")["rainrate"].mean().reset_index()
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title="Discharge [m³/s]", gridcolor="rgba(148, 163, 184, 0.3)"
        ),
        yaxis2=dict(
